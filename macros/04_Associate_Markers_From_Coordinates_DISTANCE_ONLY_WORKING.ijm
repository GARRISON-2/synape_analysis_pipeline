// ==================================================
// 04_Associate_Markers_From_Coordinates_DISTANCE_ONLY_WORKING.ijm
// ==================================================
//
// PURPOSE:
// This macro reads the individual puncta coordinate table from Macro 3
// and calculates proximity-based associations between synaptic marker puncta.
//
// It does NOT open images.
// It works only from the saved coordinates.
//
// --------------------------------------------------
// INPUT:
// --------------------------------------------------
...
//
// Parent folder/
//      08_Marker_Association_Analysis/
//          01_Results/
//              pairwise_postsynaptic_to_bassoon_nearest_neighbor.csv
//              bassoon_classification_by_threshold.csv
//              association_summary_by_threshold.csv
//
// --------------------------------------------------
// BIOLOGICAL LOGIC:
// --------------------------------------------------
...
//      PSD95    -> nearest Bassoon
//      Gephyrin -> nearest Bassoon
//
// Bassoon classification:
//      PSD95_only:
//          Bassoon has >=1 PSD95 within threshold and 0 Gephyrin.
//
//      Gephyrin_only:
//          Bassoon has >=1 Gephyrin within threshold and 0 PSD95.
//
//      Both_PSD95_and_Gephyrin:
//          Bassoon has >=1 PSD95 and >=1 Gephyrin within threshold.
//
//      Unassigned:
//          Bassoon has 0 PSD95 and 0 Gephyrin within threshold.
//
// --------------------------------------------------
// THRESHOLDS TESTED:
// --------------------------------------------------
...
//      0.75 um
//
// These are sensitivity-test thresholds.
// Later, we can lock one threshold for the final analysis.
//
// ==================================================

requires("1.53");

macro "04 Associate Markers From Coordinates Distance Only" {
    ...
    dirAssoc = parentDir + "08_Marker_Association_Analysis/";
    dirResults = dirAssoc + "01_Results/";

    File.makeDirectory(dirAssoc);
    File.makeDirectory(dirResults);

    pairwiseCSV = dirResults + "pairwise_postsynaptic_to_bassoon_nearest_neighbor.csv";
    bassoonCSV  = dirResults + "bassoon_classification_by_threshold.csv";
    summaryCSV  = dirResults + "association_summary_by_threshold.csv";

    // --------------------------------------------------
    // Thresholds to test, in um
    // --------------------------------------------------

    thresholds = newArray(0.30, 0.40, 0.50, 0.75);

    print("----------------------------------------");
    print("Starting Macro 4: marker association from coordinates.");
    print("Input CSV:");
    print(inputCSV);
    print("Output folder:");
    print(dirResults);
    ...
    bassoonHeader =
        ...
        "PSD95_count_within_threshold,PSD95_IDs_within_threshold," +
        "Gephyrin_count_within_threshold,Gephyrin_IDs_within_threshold," +
    ...
    // --------------------------------------------------
    // Output 1:
    // Pairwise postsynaptic marker -> nearest Bassoon
    // --------------------------------------------------

    print("----------------------------------------");
    print("Writing pairwise postsynaptic-to-Bassoon nearest-neighbor table.");
    ...
    // --------------------------------------------------
    // Output 2:
    // Bassoon classification at each threshold
    // --------------------------------------------------

    print("----------------------------------------");
    print("Writing Bassoon classification table.");
    ...
    // --------------------------------------------------
    // Output 3:
    // Summary by Image/Region/Layer/ROI/Z_group/threshold
    // --------------------------------------------------
    ...
    print("----------------------------------------");
    print("DONE.");
    print("Pairwise nearest-neighbor table:");
    print(pairwiseCSV);
    print("Bassoon classification table:");
    print(bassoonCSV);
    print("Summary table:");
    print(summaryCSV);
}
